...
export class LiveStartComponent implements OnInit, OnDestroy {
  ...
  private resolveGreetingAudioUrl(url: string): string {
    ...
    if (url.startsWith('/interview-service/') && globalThis.location?.protocol && globalThis.location?.hostname) {
      return `${globalThis.location.protocol}//${globalThis.location.hostname}:8081${url}`;
    }
